...
ORIGINAL_MODEL_PATH = "/mnt/lab/Models/pt/rwkv7-g1/rwkv7-g0a3-7.2b-20251029-ctx8192.pth" # <--- 这是你的4096模型路径
# 插值后新模型的保存路径
EXPANDED_MODEL_PATH = "/mnt/data/train/models/rwkv_state_dict_expanded_g0a3_L64_C6144_bo_init.pth" 
...
def expand_model_scheme_1(original_state_dict, original_n_layer, 
    ...
    for k in range(L):
        ...
        # Case B: 边界层 (0 或 L-1)
            ...
            if stack_last_layer:
                new_layers_config.extend([('copy', [k]), ('copy', [k])]) # 应用 [复制, 复制] 这里多复制两次最后一层，是因为第一层不插值，因为没有v0
            # else: 不动，不添加额外层
